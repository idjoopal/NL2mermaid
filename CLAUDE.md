...
| 서비스 | 한글 지원 | 비고 |
|--------|----------|------|
| mermaid.ink | ✅ | pako 인코딩, GET 방식 |
| Kroki | ✅ | POST 방식, 폰트 init 디렉티브 불필요 |

→ **mermaid.ink를 기본 provider로 채택** (설치 불필요, 코드 단순)
→ Kroki는 선택 provider (자체 호스팅 시 사용)
→ 한글 폰트 별도 지정 없이 기본 렌더링으로 충분
